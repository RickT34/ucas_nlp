...
def get_summery():
    ...
    for file in DIR_OUTPUT.glob('*.json'):
        ...
        csv.write(', '.join([file.stem]+[str(data['summery'][f]) for f in FILEDS[1:]])+'\n')
        
    csv.close()

def do_compare(*files:Path):
